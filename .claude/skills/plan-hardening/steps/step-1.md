# Step 1 — Plan Scope Review (`chook`)
...
`<tmp-vars-file>` and `<tmp-step-1-message>`.
...
```bash
atm send chook --team schook --stdin < <tmp-step-1-message>
```
...
- `FAIL` -> re-render and re-send Step 1 to `chook`
- if `chook` ACKs but returns no new fenced JSON, increment `round_index`,
