# Step 5 — Consistency Hardening (`chook`)
...
`<tmp-vars-file>` and `<tmp-step-5-message>`.
...
```bash
atm send chook --team schook --stdin < <tmp-step-5-message>
```
...
- `FAIL` -> re-render and re-send Step 5 to `chook`
