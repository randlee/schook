...
description: >
  ...
  the `schook` team still exists on disk and the saved `leadSessionId` still
  matches the current `SESSION_ID`, but `SendMessage` or teammate reachability
  is broken.
---
...
- `SESSION_ID` still matches `leadSessionId` in
  `~/.claude/teams/schook/config.json`
...
```text
SendMessage(to="<claude-teammate>", message="ping: verify schook communications path")
```
...
```bash
atm teams restore schook --from "$BACKUP_PATH"
```
