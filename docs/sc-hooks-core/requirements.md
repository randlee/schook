...
| `REQ-SHK-CORE-003` | Implemented | The core crate shall own canonical session/root/state types and the invariants on those types, while leaving storage policy and logging ownership to higher layers. | `REQ-SHK-HKR-008`, `REQ-SHK-HKR-012` | `sc-hooks-core/src/session.rs`, `sc-hooks-core/src/storage.rs` |
