...
| HKR-018 | planned | `docs/plan-phase-Q.md`, `docs/phase-Q/sprint-Q8.md`, `docs/phase-Q/sprint-Q9.md`, `docs/architecture.md` section `9.5`, `test-harness/hooks/opencode/`, `test_harness/hooks/opencode/`, `docs/hook-api/opencode-agent-hook-api.md` | `Q.8` and `Q.9` define one harness-only opencode follow-on; `Q.8` lands the maintained harness tree, a non-empty approved manifest for the retained `session.idle` surface, the paired Python-package root, and provider tests, while `Q.9` closes the provider-local models and current API doc | opencode harness/doc-model support is planned for Phase Q only; runtime parity remains deferred by contract. |
