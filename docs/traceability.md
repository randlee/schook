...
| HKR-019 | planned | `docs/plan-phase-Q.md`, `docs/phase-Q/sprint-Q1.md` through `docs/phase-Q/sprint-Q5.md`, `docs/architecture.md` section `9.2e`, `justfile`, `.just/`, `.github/workflows/ci.yml` | `Q.1` through `Q.5` define one curated smoke surface with a repo-owned runner, an offline CI ownership path that does not require provider CLIs, and provider-specific accepted-baseline live smoke records for Claude, Codex, and Gemini | the smoke surface is planned as a provider-validation gate only and does not authorize new runtime scope by itself |
