...
| HKR-008 | deferred | `plugins/agent-session-foundation` (pending S10-R1) | no implementation tests yet — `CLAUDE_PROJECT_DIR` confirmed as hook-only env injection (see Requirement Amendment Notes); full canonical session-state record with atomic persistence, root-equality check, and normalized consumer output deferred to S10-R1 | full implementation deferred to S10-R1 |
