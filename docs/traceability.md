...
| HKR-019 | planned | `docs/plan-phase-Q.md`, `docs/phase-Q/sprint-Q1.md` through `docs/phase-Q/sprint-Q5.md`, `docs/architecture.md` section `9.2e`, `docs/traceability.md`, `justfile`, `.just/`, `.github/workflows/ci.yml` | `Q.1` through `Q.5` define one curated smoke surface with a repo-owned runner, an offline CI ownership path that does not require provider CLIs, and provider-specific accepted-baseline live smoke records for Claude, Codex, and Gemini; `Q.5` is the single owning sprint that closes the final `HKR-019` traceability row after the last provider smoke path lands | the smoke surface is planned as a provider-validation gate only and does not authorize new runtime scope by itself |
