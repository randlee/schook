...
| HKR-018 | planned | `docs/plan-phase-Q.md`, `docs/phase-Q/sprint-Q8.md`, `docs/phase-Q/sprint-Q9.md`, `docs/architecture.md` section `9.5`, `test-harness/hooks/opencode/`, `test_harness/hooks/opencode/`, `docs/hook-api/opencode-agent-hook-api.md` | `Q.8` and `Q.9` define one harness-only opencode follow-on with a non-empty approved manifest, the paired evidence and Python-package roots, provider-local models, harness tests, and a current API doc while leaving runtime parity deferred | opencode harness/doc-model support is planned for Phase Q only; runtime parity remains deferred by contract. |
