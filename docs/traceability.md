...
| BND-001a | implemented | `plugins/*/Cargo.toml`, README, architecture docs, project-plan.md | source inventory inspection | |
| BND-002 | implemented | `plugins/*/Cargo.toml`, README, architecture docs, project-plan.md | release-facing docs and plugin metadata agree that the four runtime implementation source crates remain source-owned rather than bundled/preinstalled, while the nine remaining crates stay scaffold/reference | |
...
| GAP-003 | resolved | README, `docs/architecture.md`, `docs/requirements.md`, `docs/project-plan.md`, `plugins/*/Cargo.toml` | release-facing docs and plugin metadata consistently classify the four production-track runtime crates separately from the nine scaffold/reference crates without claiming any source crate is bundled/preinstalled |
