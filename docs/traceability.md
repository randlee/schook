...
| HKR-006 | planned | `docs/plan-phase-O.md`, `docs/phase-O/sprint-O3.md`, `docs/phase-O/sprint-O4.md`, `docs/phase-O/sprint-O5.md`, `crates/sc-hooks-cli/src/main.rs`, `crates/sc-hooks-core/src/normalization.rs`, `plugins/agent-session-foundation/src/lib.rs`, `crates/sc-hooks-cli/tests/codex_runtime_parity.rs` | `O.4` proves the approved Codex `SessionStart` and `PreToolUse(Bash)` surfaces now enter the generic runtime through the sealed normalization boundary; Gemini remains assigned to `O.5`, and Cursor stays deferred under `HKR-007` | Codex portion implemented in `O.4`; Gemini runtime parity remains planned in `O.5`. |
...
| HKR-010 | planned | `docs/plan-phase-O.md`, `docs/phase-O/sprint-O4.md`, `docs/phase-O/sprint-O5.md`, `docs/phase-O/sprint-O6.md`, `crates/sc-hooks-cli/src/main.rs`, `crates/sc-hooks-core/src/normalization.rs`, `plugins/atm-extension/src/lib.rs`, `crates/sc-hooks-cli/tests/codex_runtime_parity.rs` | `O.4` proves Codex `PreToolUse(Bash)` now preserves retryable normalization failures with `recovery_hint` and ATM extension metadata enrichment through the shared runtime path; Gemini parity remains assigned to `O.5` | Codex portion implemented in `O.4`; Gemini parity remains planned in `O.5`. |
