...
| HKR-007 | planned | `docs/plan-phase-Q.md`, `docs/phase-Q/sprint-Q6.md`, `docs/phase-Q/sprint-Q7.md`, `docs/hook-api/cursor-agent-hook-api.md`, `docs/architecture.md` sections `9.2d` and `9.4`, `test-harness/hooks/cursor-agent/`, `test_harness/hooks/cursor_agent/`, `test-harness/hooks/README.md` | `Q.6` and `Q.7` define one harness-only Cursor Agent follow-on with a non-empty approved manifest, the paired evidence and Python-package roots, provider-local models, harness tests, and a current API doc while leaving runtime parity deferred; `Q.6` lands the maintained evidence tree, non-empty approved manifest, harness tests, and the paired `cursor_agent` package root, while `Q.7` closes the provider-local models and final API-doc ownership | Cursor harness/doc-model support is planned for Phase Q only; runtime parity remains deferred by contract. |
