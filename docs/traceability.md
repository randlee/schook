...
| HKR-019 | implemented | `docs/plan-phase-Q.md`, `docs/phase-Q/sprint-Q1.md` through `docs/phase-Q/sprint-Q5.md`, `docs/phase-Q/smoke-claude.md`, `docs/phase-Q/smoke-codex.md`, `docs/phase-Q/smoke-gemini.md`, `docs/architecture.md` section `9.2e`, `docs/traceability.md`, `justfile`, `.just/`, `.github/workflows/ci.yml` | `Q.1` rejected `openshell` for now, `Q.2` closed the repo-owned `just smoke` entrypoint plus the offline CI smoke gate, and `Q.3` / `Q.4` / `Q.5` closed the accepted-baseline live smoke records for Claude, Codex, and Gemini respectively; `Q.5` is the single owning sprint that closes the final `HKR-019` traceability row after the last provider smoke path landed | the smoke surface is now implemented as a provider-validation gate only; it does not authorize new runtime scope by itself |
