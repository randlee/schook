...
| HKR-019 | Partially Implemented | `docs/plan-phase-Q.md`, `docs/phase-Q/sprint-Q1.md` through `docs/phase-Q/sprint-Q5.md`, `docs/architecture.md` section `9.2e`, `justfile`, `.just/run_smoke.py`, `.just/smoke/`, `.github/workflows/ci.yml`, `docs/phase-Q/smoke-surface.md` | `Q.2` closed the public `just smoke` entrypoint, the repo-owned smoke runner, the offline CI ownership path, and the smoke-surface contract docs; `Q.3` through `Q.5` remain the owning provider sprints for the accepted-baseline live smoke records that finish `HKR-019` | the smoke surface is partially implemented as the shared provider-validation gate and still does not authorize new runtime scope by itself |
