...
| HKR-007 | Implemented | `docs/plan-phase-Q.md`, `docs/phase-Q/sprint-Q6.md`, `docs/phase-Q/sprint-Q7.md`, `docs/hook-api/cursor-agent-hook-api.md`, `docs/architecture.md` sections `9.2d` and `9.4`, `test-harness/hooks/cursor-agent/`, `test_harness/hooks/cursor_agent/`, `test-harness/hooks/README.md` | `Q.6` and `Q.7` define one harness-only Cursor Agent follow-on with a non-empty approved manifest, the paired evidence and Python-package roots, provider-local models, harness tests, and a current API doc while leaving runtime parity deferred; `Q.6` lands the maintained evidence tree, non-empty approved manifest, harness tests, the shared harness README update, and the paired `cursor_agent` package root, while `Q.7` closes the provider-local models and final API-doc ownership | Cursor harness/doc-model support is implemented through `Q.6` and `Q.7`; runtime parity remains deferred by contract beyond the current harness-only scope. |
...
| HKR-017 | implemented | `docs/phase-N/plan-remediation.md`, `docs/phase-N/sprint-N5.md` through `docs/phase-N/sprint-N10.md`, `docs/phase-P/sprint-P1.md`, `docs/phase-P/sprint-P2.md`, `docs/hook-api/codex-hook-api.md`, `docs/hook-api/gemini-hook-api.md`, `test-harness/hooks/codex/`, `test-harness/hooks/gemini/` | the shared provider-harness contract now covers both the accepted Phase N follow-on verification baseline and the completed Phase P missing-surface expansion; `P.1` closed the Codex side by proving `notify` and documenting evidence-backed `Stop` / `resume` / `fork` disposition on the same approved harness contract, while `P.2` closed the Gemini `AfterAgent` side | no new runtime provider scope is authorized by `HKR-017` alone; runtime parity remains owned by later accepted execution sprints |
| HKR-018 | implemented | `docs/plan-phase-Q.md`, `docs/phase-Q/sprint-Q8.md`, `docs/phase-Q/sprint-Q9.md`, `docs/architecture.md` section `9.5`, `test-harness/hooks/opencode/`, `test_harness/hooks/opencode/`, `docs/hook-api/opencode-agent-hook-api.md` | the opencode harness-only follow-on is complete: `Q.8` landed the maintained harness tree, the non-empty approved manifest for the retained `session.idle` surface, the paired Python-package root, and provider tests, and `Q.9` closed the provider-local models, registry, payload-model tests, and current API doc | opencode harness/doc-model support is implemented for Phase Q only; runtime parity remains deferred by contract. |
| HKR-019 | Implemented | `docs/plan-phase-Q.md`, `docs/phase-Q/sprint-Q1.md` through `docs/phase-Q/sprint-Q5.md`, `docs/architecture.md` section `9.2e`, `docs/traceability.md`, `justfile`, `.just/`, `.github/workflows/ci.yml`, `docs/phase-Q/smoke-surface.md` | `Q.1` through `Q.5` define one curated smoke surface with a repo-owned runner, an offline CI ownership path that does not require provider CLIs, and provider-specific accepted-baseline live smoke records for Claude, Codex, and Gemini; `Q.2` closes the public `just smoke` entrypoint, the repo-owned smoke runner, the offline CI ownership path, and the smoke-surface contract docs, while `Q.3` / `Q.4` / `Q.5` close the accepted-baseline live smoke records for Claude, Codex, and Gemini respectively, with `Q.5` serving as the single closing sprint for the requirement | the smoke surface is implemented as the shared provider-validation gate and does not authorize new runtime scope by itself |
