...
| HKR-017 | implemented | `docs/phase-N/plan-remediation.md`, `docs/phase-N/sprint-N5.md` through `docs/phase-N/sprint-N10.md`, `docs/architecture.md` section `9.2d`, `justfile`, `test-harness/hooks/`, `test_harness/hooks/`, `docs/hook-api/` | the Phase N follow-on verification track closed one shared external harness contract across Claude, Codex, and Gemini with current provider API docs, provider models, approved fixtures/tests, and stable `just` targets | |
