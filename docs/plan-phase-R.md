...
| 1 | critical-plan-reviewer contract (sonnet) | `6d86d2a` | FAIL — 3 Blocking (PLAN-CRIT-001: the `SC_ATM_*` env namespace was invented — AQ2.5 names no env vars, the baseline uses `SCHOOK_CODEX_IDLE_*`, and the repo's canonical convention is `SC_HOOKS_*`; the "AQ2.5 authoritative" framing was dishonest; 002: R3 AC5's "boundaries lint" gate referenced a check `sc-lint-boundary` cannot perform — attribute lints only, no Cargo dep-graph analysis; 003: "byte-identical" parity was unachievable — `TeamMemberHeartbeatRequest` carries `pid` and `observed_at`), 2 Important (004: uniform `queue_get=true` flip would drain Codex FIFOs with no injection surface — silent nudge loss; 005: R4's retirement grep gate named no pattern and couldn't — AQ2.5's script paths don't exist yet), 1 minor (Windows cutover scope unstated) | Fixed in round-1 fix commit: env names rebased into the canonical namespace as `SC_HOOKS_ATM_*` with honest origination ("originated here, by schook; atm-core must adopt or alias — tracked coordination task"); AC5's lint clause dropped — diff-empty is the sole, stated enforcement; parity comparison defined as byte-identical after masking `pid` + all timestamp fields, masked-list enumerated and asserted non-empty per request type; `queue_get` made Claude-only with the Codex generator hard-off until an injection surface exists (R.2a contract + R.2b scope + AC updated); R4 gate restated structurally (no generated entry resolving under `scripts/hooks/`; lexical pattern finalized in-sprint); Windows cutover explicitly deferred in R4 Out of Scope. |
| 2 | both reviewer contracts (sonnet) | `d21c8c7` | **PASS — closing round** (both dimensions, all five sprints) — every round-1 closure verified against real repo state (`SC_HOOKS_*` namespace confirmed real and populated in `sc-hooks-cli/src/config.rs` + `sc-hooks-core/src/storage.rs`; `lint_sc_boundary.py` confirmed attribute-only; `protocol.rs:330-342` confirmed `pid` + `observed_at` are exactly the nondeterministic fields); round-1 scope closures intact; zero findings | Two non-gating notes carried: (1) backfill the atm-core naming-adoption issue number into sprint-R2 once filed; (2) enumerate the queue-get envelope's masked fields in R3's parity harness once AQ2.5 ships the type, before R3 AC 1 closes. Phase-R plan hardening complete; ready for dev/QA execution. |
