...
| `REQ-SHK-CLI-008` | Deferred | The CLI crate shall own layered observability-config loading across built-in defaults, global `~/.sc-hooks/config.toml`, repo-local `.sc-hooks/config.toml`, and environment overrides. | `REQ-SHK-DEF-010`, `REQ-SHK-DEF-011`, `REQ-SHK-DEF-018` | `sc-hooks-cli/src/config.rs`, `sc-hooks-cli/src/main.rs`, future config tests |
| `REQ-SHK-CLI-009` | Deferred | The CLI crate shall own observability-mode selection, full-audit sink orchestration, retention pruning, and degraded-path handling without affecting hook execution; this future audit-trail work remains distinct from the existing static `sc-hooks audit` command. | `REQ-SHK-DEF-012`, `REQ-SHK-DEF-013`, `REQ-SHK-DEF-014`, `REQ-SHK-DEF-015`, `REQ-SHK-DEF-016`, `REQ-SHK-DEF-017` | `sc-hooks-cli/src/observability.rs`, `sc-hooks-cli/src/dispatch.rs`, future audit tests |
