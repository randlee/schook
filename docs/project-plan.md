...
| Phase P | Planned | missing-hook parity expansion — complete the deferred Codex/Gemini hook surfaces in the harness first, adopt the available `sc-lint` suite through the `../atm-core` `just lint` pattern with an explicit cross-platform gate, then extend runtime normalization/parity for those surfaces, and finally close the remaining active implementation-gap rulings and release-operator follow-ons | `HKR-006`, `HKR-010`, `HKR-017`, `PRR-009`, `LOGR-QA-004`, `RULING-NEEDED-ECR-001`, `RULING-NEEDED-ECR-002`, `RULING-NEEDED-NT-CLI-002`, `RULING-NEEDED-HRN-005`, `RULING-NEEDED-COW-003` | Phase O | `docs/plan-phase-P.md`, `docs/phase-P/`, `test-harness/hooks/`, `test_harness/hooks/`, `docs/hook-api/`, `justfile`, `.just/`, `docs/cross-platform-guidelines.md`, `crates/sc-hooks-core/`, `crates/sc-hooks-cli/`, `plugins/`, install/alias docs |

Phase N closure note:

- `Phase N` and `Phase N Follow-On` merged to `develop` via PR #127 at `8891c3d` (2026-05-24)
- PARTIAL_GO verdict recorded in `docs/phase-N/readiness.md`; all N.1–N.10 sprints ACCEPTED
- approved surfaces: Codex (`SessionStart`, `PreToolUse`), Gemini (`SessionStart`, `SessionEnd`, `BeforeAgent`, `BeforeTool`, `AfterTool`), Claude (all locally exercisable surfaces)
- `docs/phase-N/readiness.md` is the authoritative verdict record
