...
| Phase Q | Planned | production-readiness smoke and next-provider harness expansion — evaluate `openshell`, add the curated `just smoke` surface and CI ownership path, land executable smoke coverage for Claude/Codex/Gemini, then expand the permanent provider harness/docs/models for Cursor Agent and opencode without yet authorizing their runtime parity work | `HKR-007`; opencode requirement IDs pending first execution sprint control-doc additions; `RULING-NEEDED-ECR-002` may inform smoke-policy wording | Phase P | `docs/plan-phase-Q.md`, `docs/phase-Q/`, `justfile`, `.just/`, `.github/workflows/ci.yml`, `test-harness/hooks/`, `test_harness/hooks/`, `docs/hook-api/`, smoke operator docs |
