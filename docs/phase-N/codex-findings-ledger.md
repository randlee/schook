...
| `CDX-N1-009` | `newtype candidates` | `session_id`, `thread-id`, `turn-id`, and `tool_use_id` behave as semantic identifiers and should not be treated as display-only strings in later Rust work. | low | `carry to N.3` | `tool_use_id` is an opaque vendor string, not a UUID contract. `transcript_path` is a provider-local path value, not a canonical identifier. |
| `CDX-N1-010` | `enum candidates` | `permission_mode` and `source` already behave like closed-shape provider enums in approved `N.1` fixtures and should be reviewed as enum candidates during `N.3` mapping. | low | `carry to N.3` | Observed values in approved fixtures are `permission_mode=bypassPermissions` and `source=startup`; later provider captures may widen the set. |
