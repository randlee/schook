...
- crates.io publication of `sc-hooks-cli` while the external `sc-observability`
  crates required by the CLI remain unpublished; that publication boundary is an
  explicit external gate rather than a local release-doc ambiguity
...
| OBS-006 | Implemented | Must | Structured observability integration shall use the external `sc-observability` crates through version-pinned dependencies owned by `sc-hooks-cli`; local validation may resolve them from the sibling `../sc-observability` checkout until crates.io publication finishes. | `sc-hooks-cli` depends on the external observability crates directly and does not use ad hoc in-workspace logger code. |
