...
| DEF-017a | Planned | Must | The `full`-mode pre-dispatch degraded-path contract shall be introduced by the full-audit phase rather than reusing the `standard` degraded stderr line. | `SC-LOG-S3` negative-branch tests prove `mode = "full"` does not emit the `standard` degraded stderr line, and `SC-LOG-S4` owns the follow-on full-mode contract. |
