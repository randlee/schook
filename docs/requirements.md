...
- bundled or preinstalled runtime plugin behavior from the source crates under
  `plugins/`; four source crates are production-track runtime implementation
  crates with direct tests, but all `plugins/` crates remain source-owned rather
  than bundled release assets
...
| BND-001 | Implemented | Must | The source crates under `plugins/` shall be documented with an explicit maturity level: scaffold/reference or runtime implementation with direct tests, and those classifications shall agree across the control docs. | The control docs classify `agent-session-foundation`, `agent-spawn-gates`, `atm-extension`, and `tool-output-gates` as production-track runtime implementation source crates with direct tests, while the remaining source crates stay scaffold/reference. |
| BND-001a | Implemented | Must | The documented plugin inventory and maturity map shall match the actual source crates in `plugins/`: `agent-session-foundation`, `agent-spawn-gates`, `atm-extension`, `tool-output-gates`, `audit-logger`, `conditional-source`, `event-relay`, `guard-paths`, `identity-state`, `notify`, `policy-enforcer`, `save-context`, and `template-source`. | Requirements, architecture, project plan, README, and plugin metadata enumerate the same thirteen source crates and distinguish the four production-track runtime implementation crates from the nine scaffold/reference crates. |
...
| --- | --- | --- | --- |
| DEF-001 | Should | Bundled or preinstalled plugin behavior beyond the current source-owned plugin inventory | Real runtime behavior, tests, and installation story exist |
...
This section records the hook-runtime program state plus the remaining deferred
follow-on items after the dispatcher foundation above. Implemented hook-runtime
crates are part of the current source inventory; deferred items stay explicitly
outside the approved release baseline.
