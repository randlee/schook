...
| HKR-008 | Deferred | Must | The generic session foundation shall persist one canonical session-state record keyed by `session_id`, `active_pid`, and `ai_root_dir`, where `ai_root_dir` is established from the first root-establishing `SessionStart` payload for the runtime instance, `ai_current_dir` is chained from each hook payload `cwd`, and downstream consumers always receive normalized project-root context even when Claude varies its raw env surface. `CLAUDE_PROJECT_DIR`, when present in the hook process environment, is a cross-check against the persisted canonical root rather than the source of truth. | Hook lifecycle code writes one canonical session record, uses the root-establishing `SessionStart` `cwd` as the immutable root for that runtime instance, preserves later `cwd` snapshots separately, never rewrites root identity from later `cwd` drift, emits prominent error-level observability when inbound `CLAUDE_PROJECT_DIR` diverges from the persisted canonical root, and exposes the canonical root back to consumers as normalized project-root context. |
