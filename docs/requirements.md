...
| HKR-004 | Deferred | Must | The initial Claude hook implementation scope shall cover only the currently verified eight-hook ATM baseline: `SessionStart`, `SessionEnd`, `PreToolUse(Bash)`, `PostToolUse(Bash)`, `PreToolUse(Agent)`, `Notification(idle_prompt)`, `PermissionRequest`, and `Stop`. | Hook crates and tests map only to that verified eight-hook set unless requirements are explicitly expanded later; `Notification(idle_prompt)` remains documented as wired-but-unresolved in local capture, and the clean runtime design is frozen in `docs/phase-bc-hook-runtime-design.md`. |
