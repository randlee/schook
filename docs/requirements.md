...
| HKR-006 | Implemented | Must | Provider-specific docs and harness-planning artifacts for Codex, Gemini, and Cursor may be kept in the docs set before implementation, but those providers shall not block or precede the first Claude implementation path. After the Claude baseline is stable, Codex and Gemini may proceed through an explicitly approved harness-planning phase. `Phase O` is the approved runtime-normalization phase for Codex and Gemini only; Cursor runtime work remains deferred. | Acceptance conditions satisfied: (1) `Phase N` closed the harness-planning gate by proving Codex and Gemini provider evidence, approved fixtures, provider models, and provider API docs without authorizing runtime work. (2) `O.4` closed the Codex runtime-normalization gate for the approved `SessionStart` and `PreToolUse(Bash)` surfaces by routing them through the normalized runtime path. (3) `O.5` closed the remaining approved Gemini runtime-normalization surfaces. (4) `O.6` froze the shared Claude/Codex/Gemini plugin-path parity proof and the stable cross-provider observability shape on the approved overlapping surfaces. (5) Cursor remains deferred under `HKR-007`. |
