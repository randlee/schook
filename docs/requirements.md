...
| BND-001a | Implemented | Must | The documented plugin inventory and maturity map shall match the actual source crates in `plugins/`: `agent-session-foundation`, `agent-spawn-gates`, `tool-output-gates`, `audit-logger`, `conditional-source`, `event-relay`, `guard-paths`, `identity-state`, `notify`, `policy-enforcer`, `save-context`, and `template-source`. | Architecture and plan docs enumerate the same source-crate set and identify `agent-session-foundation`, `agent-spawn-gates`, and `tool-output-gates` as the non-scaffold runtime crates in this branch. |
