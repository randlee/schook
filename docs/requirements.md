...
| `Partially Implemented` | Some planned behavior is present in code or docs, but the full gated requirement is not yet satisfied |
...
| HKR-002 | Required Before Release | Must | No hook implementation branch may merge until the Claude hook schema harness captures and validates the required Claude payloads for the planned hook set. Any earlier implementation branch must be labeled as a pre-gate prototype blocked from merge. | The first hook-development sprint produces captured Claude fixtures and validation models before any hook runtime branch is promoted beyond prototype status. |
| HKR-003 | Required Before Release | Must | After the Claude schema harness captures real payloads, the plan and hook API docs shall be revised from captured evidence before any hook implementation branch is treated as merge-ready. | `docs/plugin-plan-s9.md` and the hook API docs are updated from captured fixtures before the first merge-ready hook crate lands. |
| HKR-004 | Partially Implemented | Must | The initial Claude hook implementation scope shall cover only the currently verified eight-hook ATM baseline: `SessionStart`, `SessionEnd`, `PreToolUse(Bash)`, `PostToolUse(Bash)`, `PreToolUse(Task)`, `Notification(idle_prompt)`, `PermissionRequest`, and `Stop`. Prototype branches may land subsets of that baseline only when they are explicitly marked pre-gate and blocked from merge until `HKR-002` and `HKR-003` close. | Hook crates and tests map only to the verified eight-hook set, and any pre-gate prototype branch is clearly marked blocked-from-merge until the harness and plan-revision gates are complete. |
