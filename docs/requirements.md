...
- shipped runtime plugin behavior from the legacy scaffold/reference crates under `plugins/`; `agent-session-foundation`, `agent-spawn-gates`, `tool-output-gates`, and `atm-extension` are the current runtime-implementation crates in this branch
...
| DEF-008 | Partially Implemented | Should | The first observability expansion after file-sink contract coverage shall add console-sink dispatch tests through the real `sc-hooks-cli` path, proving success, block, error, and timeout emission without weakening the current file-sink contract. | Real-dispatch console-sink tests now cover success, block, invalid-json error, and timeout; remaining deferred work is limited to custom sinks and multi-hook monitoring correlation. |
| BND-001 | Implemented | Must | The source crates under `plugins/` shall stay documented as either scaffold/reference code or non-scaffold runtime implementation code without implying shipped bundled-plugin status. | Current release-facing docs keep the 9 legacy plugin crates scaffold/reference only, while `agent-session-foundation`, `agent-spawn-gates`, `tool-output-gates`, and `atm-extension` are documented as the current non-scaffold runtime crates in this branch. |
| BND-001a | Implemented | Must | The documented plugin inventory shall match the actual source crates in `plugins/`: `agent-session-foundation` (runtime, non-scaffold), `agent-spawn-gates` (runtime, non-scaffold), `atm-extension` (runtime, non-scaffold), `tool-output-gates` (runtime, non-scaffold), `audit-logger` (scaffold/reference), `conditional-source` (scaffold/reference), `event-relay` (scaffold/reference), `guard-paths` (scaffold/reference), `identity-state` (scaffold/reference), `notify` (scaffold/reference), `policy-enforcer` (scaffold/reference), `save-context` (scaffold/reference), and `template-source` (scaffold/reference). | Architecture and plan docs enumerate the same 13 source crates and keep the 4 Sprint 9 hook-runtime crates distinct from the 9 legacy scaffold/reference crates without claiming bundled installed plugins. |
| BND-002 | Implemented | Must | Any bundled plugin described as shipped functionality shall have direct behavior tests and runtime installation guidance. Source-only implementation crates may land before install guidance, but they must not be described as preinstalled runtime plugins. | Release-facing docs keep runtime discovery at `.sc-hooks/plugins/` and avoid any bundled installed-plugin claim for the current source crates. |
...
| HKR-003 | Implemented | Must | After the Claude schema harness captures real payloads, the plan and hook API docs shall be revised from captured evidence before implementation begins. | `docs/plugin-plan-s9.md` and the hook API docs were revised from captured fixtures, including `resume` and `clear` follow-up evidence. |
...
| HKR-008 | Deferred | Must | The generic session foundation shall persist one canonical session-state record keyed by `session_id`, `active_pid`, and `ai_root_dir`, where `ai_root_dir` is chained from `CLAUDE_PROJECT_DIR`, `ai_current_dir` is chained from the payload `cwd`, and both fields are reused by later hook calls. | Hook lifecycle code writes one canonical session record, does not fall back to cwd for root identity, preserves the current-directory snapshot from each lifecycle payload, and later hook calls resolve the same session state through the persisted identity tuple. |
| HKR-009 | Deferred | Must | Canonical hook session-state updates shall use atomic write semantics, shall not rewrite `session.json` when the canonical record is unchanged, and shall emit hook logs on every invocation whether or not state changes. | Session-state persistence uses same-directory temp-plus-rename, increments revision only on material change, skips unchanged rewrites, and still emits observability/log output for every hook invocation. |
